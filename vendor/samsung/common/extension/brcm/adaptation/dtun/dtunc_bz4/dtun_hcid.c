...
#ifdef U1_JPN_NTT
int dtunops_set_av_streaming (int index, boolean av_streaming)
{
    tDTUN_DEVICE_METHOD method;

    method.set_av_streaming.hdr.id = DTUN_METHOD_DM_SET_AV_STREAMING;
    method.set_av_streaming.hdr.len
        = sizeof(tDTUN_METHOD_DM_SET_AV_STREAMING) - sizeof(tDTUN_HDR);
    method.set_av_streaming.av_streaming = av_streaming;
...
static struct btd_adapter_ops dtun_ops = {
	...
	#ifdef U1_JPN_NTT
	.set_av_streaming = dtunops_set_av_streaming
	#endif
};
