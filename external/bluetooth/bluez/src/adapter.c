...
#ifdef U1_JPN_NTT
static DBusMessage *set_av_streaming(DBusConnection *conn,
                                     DBusMessage *msg,
                                     gboolean av_streaming,
                                     void *data)
{
    struct btd_adapter *adapter = data;
    int err;

    err = adapter_ops->set_av_streaming(adapter->dev_id, av_streaming);
    if (err < 0)
        return failed_strerror(msg, -err);
...
static int adapter_start_inquiry_by_cod(struct btd_adapter *adapter, int cod)
{
	gboolean periodic = TRUE;
...
// U1_KOR_SKT BT COD(Class of Device) end
...
// U1_KOR_SKT BT_COD Feature start
static DBusMessage *adapter_start_discovery_by_cod(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	struct session_req *req;
	struct btd_adapter *adapter = data;
	...
	err = adapter_start_inquiry_by_cod(adapter, cod);
	if (err < 0)
		return failed_strerror(msg, -err);
					...
					DBusMessage *msg, void *data)
{
	...
	#ifdef U1_JPN_NTT
	} else if (g_str_equal("AvStreaming", property)) {
        	gboolean av_streaming;

	        if (dbus_message_iter_get_arg_type(&sub) != DBUS_TYPE_BOOLEAN)
        	    return invalid_args(msg);

	        dbus_message_iter_get_basic(&sub, &av_streaming);

        	return set_av_streaming(conn, msg, av_streaming, data);
	#endif
	}
...
#if 1 //  U1_KOR_SKT BT_COD Feature
	{ "StartDiscoveryByCod",	"i",	"",	adapter_start_discovery_by_cod },
#endif
...
#define COD_MAJOR_HID          0x0500  //COD_HID
...
// U1_KOR_SKT BT COD(Class of Device) start
	...
	if (cod_filter == DEVICE_HEADSET) {
        major = dev->class & COD_MAJOR_MASK;
        if (major != COD_MAJOR_AUDIO_VIDEO && major != COD_MAJOR_WEARABLE) {
            return;
        }
    }
    else if (cod_filter == DEVICE_PC) {
        if ((dev->class & COD_MAJOR_MASK) != COD_MAJOR_COMPUTER) {
            return;
        }
    }
    else if (cod_filter == DEVICE_PHONE) {
        if ((dev->class & COD_MAJOR_MASK) != COD_MAJOR_PHONE) {
            return;
        }
    }
    //COD_HID
    else if (cod_filter == DEVICE_HID) {
        if ((dev->class & COD_MAJOR_MASK) != COD_MAJOR_HID) {
            return;
        }
    }
// U1_KOR_SKT BT COD(Class of Device) end
