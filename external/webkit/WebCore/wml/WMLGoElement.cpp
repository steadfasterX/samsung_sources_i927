...
void WMLGoElement::prepareGETRequest(ResourceRequest& request, const KURL& url)
{
    ...
    // SAMSUNG_WML_FIXES+url
    // http://218.248.40.195/vswap2/interface/ExamineTest?id=wml/events/intrinsics/1&ts=0051
    if (m_postfieldElements.size()>0) {
        KURL remoteURL(url);
	// Fix for china monternet homepage - When form action url has a query string, append the form data to the existing query string
	String query = remoteURL.query(); 
	if(query.isEmpty())
	    query = data->flattenToString();
	else
	    query = query + "&" + data->flattenToString();
