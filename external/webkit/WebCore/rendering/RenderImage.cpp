...
void RenderImage::paintReplaced(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    if (!hasImage() || errorOccurred()) {
        ...
        if (cWidth > 2 && cHeight > 2) {
            ...
            if (errorOccurred() && (m_altText.isEmpty() || notextdraw) && !image()->isNull() && (usableWidth >= image()->width()) && (usableHeight >= image()->height())) {
            ...
            if (!m_altText.isEmpty()) {
                ...
                if (errorPictureDrawn) {
                    if (cWidth >= textWidth && font.height() <= imageY)
                        context->drawText(style()->font(), textRun, IntPoint(ax, ay + ascent));
                } else if (cWidth >= textWidth && cHeight >= font.height())
                    context->drawText(style()->font(), textRun, IntPoint(ax, ay + ascent));
            }
        }
