...
//SAMSUNG CHANGES >>
void LayerAndroid::copyAnimations(const LayerAndroid* l)
{ 
    removeAnimationsForProperty(AnimatedPropertyWebkitTransform);
    KeyframesMap::const_iterator end = l->m_animations.end();
	for (KeyframesMap::const_iterator it = l->m_animations.begin(); it != end; ++it) {
  		if(it->second->type() == AnimatedPropertyWebkitTransform){	
       		m_animations.add(it->first, it->second);
  		}
  	}
}
...
bool LayerAndroid::drawChildrenCanvas(SkCanvas* canvas)
{
    ...
    if (count > 0) {
        ...
        for (int i = 0; i < count; i++)
        {
            LayerAndroid* childLayer = this->getChild(i);
	    if(childLayer!=NULL)
	            sublayers.append(this->getChild(i));
        }
        // now we sort for the transparency
