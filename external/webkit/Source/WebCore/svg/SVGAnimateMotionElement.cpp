...

{
    ...
    if (!isAdditive())
		...
		if (isAccumulated() && repeat)
			transform->translate(m_toPoint.x() * repeat, m_toPoint.y() * repeat);
#endif // ENABLE(SFB_SVG)
    ...
    if (animationMode() == PathAnimation) {
        ...
        Path path = animationPath();
				...
				if (isAccumulated() && repeat) {
					bool t;
					FloatPoint finalPosition = path.pointAtLength(path.length(), t);
					if(t)
						transform->translate(finalPosition.x() * repeat, finalPosition.y() * repeat);
				}
#endif // ENABLE(SFB_SVG)
